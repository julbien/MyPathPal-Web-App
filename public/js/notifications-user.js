...
async function fetchCSRFToken() {
    try {
        const res = await fetch('/api/user/csrf-token', { method: 'GET', credentials: 'include' });
...
async function loadNotifications() {
    try {
        const response = await fetch('/api/user/notifications?limit=100', { credentials: 'include' });
...
function updateNavbarUnreadBadge() {
    ...
    const markAllBtnNavbar = document.getElementById('markAllReadBtn');
    if (badge) {
        if (unreadCount > 0) { badge.textContent = unreadCount > 99 ? '99+' : unreadCount; badge.style.display = 'flex'; }
        else { badge.style.display = 'none'; }
    }
    if (markAllBtnNavbar) markAllBtnNavbar.style.display = unreadCount > 0 ? 'block' : 'none';
}
...
function renderNavbarDropdown(notifications) {
        ...
        if (notificationsList) {
            ...
            notificationsList.querySelectorAll('.notification-item.unread').forEach(el => {
                el.addEventListener('click', async () => {
                    ...
                    if (!id) return;
                    try {
                        await fetch(`/api/user/notifications/${id}/read`, { method: 'PUT', headers: { 'X-CSRF-Token': csrfToken }, credentials: 'include' });
                        const n = allNotifications.find(n => String(n.notification_id) === String(id));
                        if (n) n.is_read = true;
                        renderNavbarDropdown(allNotifications);
                    } catch (e) { console.error('User: mark one failed', e); }
                });
            });
        }
    ...
    } catch (e) { console.error('User: render navbar dropdown failed', e); }
}
...
async function markAllNotificationsAsRead() {
    try {
        ...
        const res = await fetch('/api/user/notifications/mark-all-read', { method: 'PUT', headers: { 'X-CSRF-Token': csrfToken }, credentials: 'include' });
        if (!res.ok) throw new Error('Failed to mark all read');
        allNotifications.forEach(n => { n.is_read = true; });
        filterNotifications(); updateNavbarUnreadBadge();
